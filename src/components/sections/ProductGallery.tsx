...
export default function ProductGallery({ compact = false }: { compact?: boolean }) {
  ...
  return <div>
    ...
    <div className="relative mt-8 overflow-hidden rounded-2xl border border-white/15 bg-[#080c18] p-1.5 shadow-[0_35px_100px_rgba(0,0,0,.45)] sm:rounded-3xl sm:p-2">
      <div className="flex items-center justify-between border-b border-white/10 px-3 py-2"><div className="flex gap-1.5"><i className="h-2 w-2 rounded-full bg-rose-400/70"/><i className="h-2 w-2 rounded-full bg-amber-300/70"/><i className="h-2 w-2 rounded-full bg-emerald-400/70"/></div><span className="text-[9px] font-bold uppercase tracking-[.18em] text-slate-500">TurboCart Windows · {isVideo ? "Product animation" : "Product screenshot"}</span><a href={active.src} target="_blank" rel="noreferrer" aria-label={`Open ${active.label} ${isVideo ? "animation" : "screenshot"} full size`} className="rounded-md p-1 text-slate-500 transition hover:text-cyan-300"><Expand className="h-4 w-4"/></a></div>
      <AnimatePresence mode="wait">{isVideo
        ? <motion.video key={active.src} initial={{opacity:0,scale:.995}} animate={{opacity:1,scale:1}} exit={{opacity:0}} transition={{duration:.25}} src={active.src} autoPlay={!reducedMotion} loop muted playsInline preload="metadata" disablePictureInPicture controlsList="nodownload noplaybackrate" aria-label="TurboCart dashboard product animation" className="aspect-video w-full rounded-b-xl bg-[#060a15] object-cover sm:rounded-b-2xl"/>
        : <motion.img key={active.src} initial={{opacity:0,scale:.995}} animate={{opacity:1,scale:1}} exit={{opacity:0}} transition={{duration:.25}} src={active.src} alt={`TurboCart Windows ${active.label} interface`} width="1920" height="1080" loading={compact?"lazy":"eager"} decoding="async" className="aspect-video w-full rounded-b-xl object-cover object-top sm:rounded-b-2xl"/>
      }</AnimatePresence>
    </div>
